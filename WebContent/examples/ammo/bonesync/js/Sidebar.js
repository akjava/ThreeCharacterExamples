var Sidebar = function ( application ) {
	...
	this.boneMoveY=0;
	this.boneMoveZ=0;
	this.targetName="ball1";
	
	var container = new UI.Panel();
	container.setId( 'sidebar' );
	container.add(new UI.AppName("Simple BoneSync"));
	
	var panel=new UI.Panel();
	container.add(panel);
	
	var listRow=UI.ListRow("Target",["ball0","ball1","ball2"],function(v){
		scope.targetName=v;
	},scope.targetName);
	panel.add(listRow);
	
	function activate(){
		application.ball0.getBody().activate();
		application.ball1.getBody().activate();
