/**
 * Controller class for the Timeliner GUI Kai.
 * modify version of TimelinerController.js
 * 
 * support channel label property for bone or morph
 *
...
THREE.TimelinerController.prototype = {
	...
	init: function( timeliner ) {
		...
		for ( var i = 0, n = trackInfo.length; i !== n; ++ i ) {
			...
			this._channelNames.push(spec.label);
			this._nameMaps[spec.label]=spec.propertyPath;
	...
	_toPropertyPath:function(channelName){
		return this._nameMaps[channelName];
	},
	...
	getChannelKeyTimes: function( channelName ) {
		return this._tracks[ this._toPropertyPath(channelName) ].times;
	...
	setKeyframe: function( channelName, time ) {
		...
		times[ index ] = time;
		this._propRefs[ this._toPropertyPath(channelName)  ].getValue( values, offset );
	...
	serialize: function() {
		...
		for ( var i = 0, n = names.length; i !== n; ++ i ) {
			...
			var name = names[ i ],pname=this._toPropertyPath(name),
				track = tracks[ pname ];
